...
class DQN(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.hl1 = nn.Linear(in_features=9, out_features=9)
        self.hl2 = nn.Linear(in_features=9, out_features=9)
        self.out = nn.Linear(in_features=9, out_features=4)
...
class Piece:
    ...
    def stating(self, position):
        ...
        self.state = self.matrix_gen(position) - past_state
    ...
    def periodic_square(self):
        ...
        if len(Piece.replay_memory) > Piece.max_length - 1:
        ...
        self.training(state, next_state, Piece.r)
        if self.iteration % Piece.update_target_net:
            target_net.load_state_dict(policy_net.state_dict())
        self.state = next_state
        self.action_number = next_action_number

    def training(self, state, next_state, reward):
        ...
        target_q = next_q * Piece.gamma + reward
...
piece = Piece(rows=3, columns=3, position=(0, 0))
piece.draw_grid()
